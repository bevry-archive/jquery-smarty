...
{	// Create our Plugin function, with $ as the argument (we pass the jQuery object over later)
	...
	{	// Our Plugin definition
		...
		data: {
			'build':'0.2.0-dev (February 19, 2008)'
		},
		...
		functions: {
			...
			{	// Assign a variable
				...
				var call = '$.Smarty.data'+$.Smarty.varloc(attributes['var'])+' = value';
				// $.Smarty.debug(call);
				eval(call);
				// $.Smarty.debug($.Smarty.data);
				// Return
				return content.populate();
			},
		...
		varloc: function ( value, prefix )
		{
			...
			value = value.replace(/\-\>/g, '.'); // turn class->var to class.var
		...
		{	// Have a value or variable
			...
			for (part in parts) { // Cycle
				...
				switch ( true )
				{
					...
					case (a === '$'):
						// Variable
						// $.Smarty.debug('value:',value, $.Smarty.data);
